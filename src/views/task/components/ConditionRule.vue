<template>
  <div>
    <div v-for="(orForm, orIdx) in conditions" :key="orIdx" class="form-condition-item-container">
      ...
      <slot name="header" :orForm="orForm"></slot>
      <div v-for="(andForm, andIdx) in orForm[ruleKey]" :key="andIdx" class="form-item-condition-item">
        ...
        <el-input
          ...
          v-show="!noThreshold(andForm.operator)"
          placeholder="请输入阈值"
          size="small"
          style="width: 50%" />
...
<script>
import { noThreshold } from '@/consts/alarm'
...
export default {
  ...
  methods: {
    ...
    addConditionOr () {
      const conditionTpl = Object.assign({}, this.conditionItemTpl)
      const item = Object.assign({}, this.ruleTpl)
      item[this.ruleKey] = [conditionTpl]
